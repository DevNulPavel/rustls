...
mod client_hello {
    use crate::crypto::{SharedSecretSink, SupportedKxGroup};
    ...
    fn emit_server_hello<C: CryptoProvider>(
        ...
        kx.complete(&share.payload.0, &mut sink)?;
        let key_schedule = if let KeyScheduleSecretSink::Output(output) = sink {
            output
        } else {
            unreachable!();
        };
